...
combined_cons = [combined[i] == xp.Sum(weight_f[c] * full.iloc[i, 17+c] for c in Categories) + 
                    ...
                    amount_w_f * full["Amount"][i] for i in Cases]
...
np.savetxt(f"!full model without month weights.csv", [p for p in zip(testing_days_used, np.array(gain_value_mem), np.array(loss_value_mem), 
