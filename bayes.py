...
pd.options.mode.chained_assignment = None  # default='warn'. Uncomment this to see warning messages.
# The warnings do not affect the code.
...
# Rename column name 'bank_to' to 'home_bank' to be consistent with the first batch
customer2 = customer2.rename(columns={"bank_to" : "home_bank"}) 
...
#### NOTE! No Streaming Services category in dataset 2. Hence create a column of 0s in full2 to allow concatenation
...
start = training.loc[training["day"] == 305] 
start_weight_c = np.zeros(20)
start_start_month_w = 0
start_mid_month_w = 0
start_end_month_w = 0
start_amount_w = 0
start_cust_w = 0.4
start_transact_w = 0.4

vec = [start_amount_w, start_transact_w, start_cust_w]
vec_mult_frame = start.iloc[:,[2,11,15]] * vec
weight_sol_proc_mult_frame = start.iloc[:,17:37] * start_weight_c
month_vec = [start_start_month_w, start_mid_month_w, start_end_month_w]
month_vec_mult_frame = start.iloc[:, -3:] * month_vec
...
start['to_investigate'] = sample


candidate_cases = start[start["to_investigate"] == 1]
...
prob_init = xp.problem('prob_init')
prob_init.setControl('outputlog', 0) # suppress output
...
prob_init.addVariable(y,int2,ext)

prob_init.addConstraint(int2[i,b] <= 2 * candidate_cases.iloc[i, 39+b] for i in Cases3 for b in Banks)
prob_init.addConstraint(ext[i] + xp.Sum(int2[i,b] for b in Banks)/2 == y[i] for i in Cases3)
prob_init.addConstraint(xp.Sum(int2[i,b]/2 * candidate_cases["time_spent"][i] for i in Cases3) <= teamsize[b] for b in Banks)
prob_init.setObjective(xp.Sum(ext[i] * ext_cost[candidate_cases["priority"][i]-1] + 
                            (1-y[i])*candidate_cases["Amount"][i] for i in Cases3), sense = xp.minimize)
prob_init.solve()
y_sol = prob_init.getSolution(y)
candidate_cases["decided_investigations"] = y_sol

int2_sol = prob_init.getSolution(int2)
ext_sol = prob_init.getSolution(ext)
candidate_cases["externally_investigated"] = ext_sol

start["decided_investigations"] = 0
start["externally_investigated"] = 0

start.loc[start["to_investigate"] == 1, "decided_investigations"] = y_sol
start.loc[start["to_investigate"] == 1, "externally_investigated"] = ext_sol

# Amount gained from detecting scams
print("Gain from the detected scams is:", sum(start[(start["decided_investigations"] == 1) & (start["is_scam"] == 1)]["Amount"]))

# Actual loss from undetected scams
print("Loss from scams undetected is:", sum(start[(start["decided_investigations"] == 0) & (start["is_scam"] == 1)]["Amount"]))
...
temp_dict = {1: 40,
                ...
                3: 100,
                4: 150}
start["ext_cost"] = start["priority"].map(temp_dict)
spending = sum(start["externally_investigated"] * start["ext_cost"])
print("Total loss + spending:",  sum(start[(start["decided_investigations"] == 0) & (start["is_scam"] == 1)]["Amount"]) + spending)
...
# Begin loop
knowledge = start[start["decided_investigations"] == 1]

n_training_days = len(np.unique(full["day"]))

testing_days_used = np.unique(training["day"])[306:] # change this number to change the no. of testing days
n_testing_days = len(testing_days_used)

n_priority_1_invst = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 1) & (start["priority"] == 1)])[0]
n_priority_2_invst = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 1) & (start["priority"] == 2)])[0]
n_priority_3_invst = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 1) & (start["priority"] == 3)])[0]
n_priority_4_invst = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 1) & (start["priority"] == 4)])[0]

n_priority_1_scam = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 0) & (start["priority"] == 1)])[0]
n_priority_2_scam = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 0) & (start["priority"] == 2)])[0]
n_priority_3_scam = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 0) & (start["priority"] == 3)])[0]
n_priority_4_scam = np.shape(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 0) & (start["priority"] == 4)])[0]

true_neg_p1 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 0) & (start["priority"] == 1)])[0]
true_neg_p2 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 0) & (start["priority"] == 2)])[0]
true_neg_p3 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 0) & (start["priority"] == 3)])[0]
true_neg_p4 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 0) & (start["priority"] == 4)])[0]

false_positive_p1 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 1) & (start["priority"] == 1)])[0]
false_positive_p2 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 1) & (start["priority"] == 2)])[0]
false_positive_p3 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 1) & (start["priority"] == 3)])[0]
false_positive_p4 = np.shape(start.loc[(start["is_scam"] == 0) & (start["decided_investigations"] == 1) & (start["priority"] == 4)])[0]
...
temp_new_df = pd.DataFrame({'test_day': 305, 'invest': start.loc[start["decided_investigations"] == 1]["transaction_id"].to_numpy()})
df_mem = pd.concat([df_mem, temp_new_df], ignore_index = True)

# Include loss on first day!!!!
loss_value_mem.append(sum(start[(start["decided_investigations"] == 0) & (start["is_scam"] == 1)]["Amount"]) + spending)
loss_value_mem_scams.append(sum(start[(start["decided_investigations"] == 0) & (start["is_scam"] == 1)]["Amount"]))
n_scam_undetected.append([n_priority_1_scam, n_priority_2_scam, n_priority_3_scam, n_priority_4_scam]) # False negative
gain_value_mem.append(sum(start[(start["decided_investigations"] == 1) & (start["is_scam"] == 1)]["Amount"]))
...
start["bank_from"] = start["bank_from"].astype('category')
bank_gain.append(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 1)].groupby(["bank_from"])["Amount"].sum().fillna(0).astype(float).to_numpy())

# scam lost pet bank
bank_loss_scams.append(start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 0)].groupby(["bank_from"])["Amount"].sum().fillna(0).astype(float).to_numpy())

# total lost per bank
temporary_ext = start.loc[start["externally_investigated"] == 1]
...
bank_loss_scams_ext.append(vec_bank_ext_loss + start.loc[(start["is_scam"] == 1) & (start["decided_investigations"] == 0)].groupby(["bank_from"])["Amount"].sum().fillna(0).astype(float).to_numpy())
...
for q in range(n_testing_days):
    ...
    test_day = testing_days_used[q]
    full = knowledge.reset_index(drop = True)
    ...
    # Combined weights
    combined_cons = [combined[i] == xp.Sum(weight_f[c] * full.iloc[i, 17+c] for c in Categories) + # Multiply weights by categories
    ...
    # Use loop to make the model keep resampling until it gets at least 1 of each priority.
    # If the model fails to investigate at least 1 of each priority after 50 tries, then carry on.
    # The justification for this is, under our ignorance of false negatives, it is best that the bank check at least one of
    # each priority. This is so that the bank does not remain ignorance of certain features of some scams.
    # By making sure that even low probability scams are checked, we can be confident that we can adjust our ignorance.
    while True:
        print("in loop...")
        counter += 1
        sample = np.random.binomial(1,combined_day2)
        ...
        if (counter <= 50) & (1 not in np.array([full2.loc[full2['to_investigate'] == 1, "priority"]])):
            continue
        if (counter <= 50) & (2 not in np.array([full2.loc[full2['to_investigate'] == 1, "priority"]])):
            continue
        if (counter <= 50) & (3 not in np.array([full2.loc[full2['to_investigate'] == 1, "priority"]])):
            continue
        if (counter <= 50) & (4 not in np.array([full2.loc[full2['to_investigate'] == 1, "priority"]])):
            continue
        ...
        prob_2nd.setControl('miprelstop', 5e-4) # Stops Branch and Bound within 0.1% of the optimal. 
        # Default value is 1e-4. Changing this value will reduce the model runtime, without significantly affecting the optimal output.

        y = np.array([xp.var(vartype = xp.binary, name = 'y_{0}'.format(i+1))
                    for i in Cases3], dtype = xp.npvar) # 0 or 1

        # int2 represents the proportion of contribution by bank j for case i. 
        # if int2 = 1, then the contribution by bank j is 1/2. If int2 = 1, then the contribution is 1.
        int2 = np.array([xp.var(vartype = xp.integer, name = 'internal_{0}_{1}'.format(i+1, j+1))
                    for i in Cases3 for j in Banks], dtype = xp.npvar).reshape(n_daily_cases3, n_banks) # will be 0, 1, or 2
    
        ext = np.array([xp.var(vartype = xp.binary, name = 'internal_{0}'.format(i+1))
                    for i in Cases3], dtype = xp.npvar).reshape(n_daily_cases3) # 0 or 1
        prob_2nd.addVariable(y,int2,ext)


        # Check what candidate_cases.iloc[i,39] is
        ...
        # print(full2.loc[full2['decided_investigations'] == 1, "priority"])
        if (counter <= 50) & (1 not in np.array([full2.loc[full2['decided_investigations'] == 1, "priority"]])):
            continue
        elif (counter <= 50) & (2 not in np.array([full2.loc[full2['decided_investigations'] == 1, "priority"]])):
            continue
        elif (counter <= 50) & (3 not in np.array([full2.loc[full2['decided_investigations'] == 1, "priority"]])):
            continue
        elif (counter <= 50) & (4 not in np.array([full2.loc[full2['decided_investigations'] == 1, "priority"]])):
            continue
        else:
            break
    ...
    print(f"Time since start of code: {end - timer:0.4f} seconds")
   
    temp_new_df = pd.DataFrame({'test_day': test_day, 'invest': full2.loc[full2["decided_investigations"] == 1]["transaction_id"].to_numpy()})
    df_mem = pd.concat([df_mem, temp_new_df], ignore_index = True)
        
    # Create blank entries if none of one of the priorities:
    current_knowledge = full2[full2["decided_investigations"] == 1]

    if 1 not in np.array(current_knowledge["priority"]):
        current_knowledge = pd.concat([current_knowledge, pd.DataFrame(0, index = np.arange(1), columns=current_knowledge.columns)], ignore_index=True)
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "priority"] = 1
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "decided_investigations"] = 1
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "day"] = current_knowledge["day"][0]
    if 2 not in np.array(current_knowledge["priority"]):
        current_knowledge = pd.concat([current_knowledge, pd.DataFrame(0, index = np.arange(1), columns=current_knowledge.columns)], ignore_index=True)
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "priority"] = 2
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "decided_investigations"] = 1
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "day"] = current_knowledge["day"][0]
    if 3 not in np.array(current_knowledge["priority"]):
        current_knowledge = pd.concat([current_knowledge, pd.DataFrame(0, index = np.arange(1), columns=current_knowledge.columns)], ignore_index=True)
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "priority"] = 3
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "decided_investigations"] = 1
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "day"] = current_knowledge["day"][0]
    if 4 not in np.array(current_knowledge["priority"]):
        current_knowledge = pd.concat([current_knowledge, pd.DataFrame(0, index = np.arange(1), columns=current_knowledge.columns)], ignore_index=True)
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "priority"] = 4
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "decided_investigations"] = 1
        current_knowledge.at[np.shape(current_knowledge)[0]-1, "day"] = current_knowledge["day"][0]
    # print(current_knowledge[-1:])
    knowledge = pd.concat([knowledge, current_knowledge[current_knowledge["decided_investigations"] == 1]], ignore_index = True)
...
testing_days_used = np.unique(training["day"])[305:]
np.savetxt(f"!bayes full v5 (trial 5).csv", [p for p in zip(testing_days_used, np.array(gain_value_mem), np.array(loss_value_mem_scams),
...
df_mem.to_csv(f"!Bayes investigations.csv", index = False)