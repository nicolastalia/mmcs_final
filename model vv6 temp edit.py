...
full = training.reset_index(drop = True)
...
# Objective and solve
prob_full.setObjective(xp.Sum(external[d,b,p] * ext_cost[p] for d in Days for b in Banks for p in Priorities) + 
                        xp.Sum((1-combined[i]) * full["is_scam"][i] * full["Amount"][i] for i in Cases), sense = xp.minimize)
prob_full.solve()
...
testing_days_used = np.unique(training["day"])[0:] # change this number to change the no. of testing days
...
for q in range(n_testing_days):
    ...
    full2 = training.loc[training["day"] == test_day]
    print("*************************************************************************")
...
np.savetxt(f"!full model with month weights.csv", [p for p in zip(testing_days_used, np.array(gain_value_mem), np.array(loss_value_mem_scams),
...
df_mem.to_csv(f"!full model with month weights investigations.csv", index = False)